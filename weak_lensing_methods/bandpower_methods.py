...
def get_cls(l_mode_index = 0, num_tom = 5, mocks_dir = None, mocks_name = None):
    ...
    cls_raw = fid_tar.read_vals(vals_to_read = 'shear_cl--bin', output_type = 'as_flat', truncate = 0).reshape(-1, len(ell))
    fid_tar.close()
    ...
    for i in range(num_tom):
        for j in range(num_tom):
            if j > i:
                cl = cls_raw[bin_order.index('bin_%s_%s' %(j+1, i+1))][l_mode_index]
            else:
                cl = cls_raw[bin_order.index('bin_%s_%s' %(i+1, j+1))][l_mode_index]
            cls[i, j] = cl
...
def bandpower_cov(ell, num_tom = 5, num_bands = 8, mocks_dir = None, mocks_name = None):
    ...
    for i in range(total_num_ell):
        theory_cls = get_cls(l_mode_index = i, num_tom = 5, mocks_dir = mocks_dir, mocks_name = mocks_name)
    ...
    band_cutoffs = np.logspace(np.log(min(ell)), np.log(max(ell)+1), num_bands+1, base = np.e)
    combined_bandpower_cov = np.zeros((120,120))
    for i in range(num_bands):
        bandpower_cov = np.zeros((num_pairs, num_pairs))
        for j in range(num_bands):
            bandpower_cov += temp_cl_cov[np.logical_and(ell >= band_cutoffs[j], ell < band_cutoffs[j+1])].sum(axis=0)
...
def make_bandpowers(mock_run, mocks_dir, mocks_name, ell_val = 'shear_cl--ell', cl_vals = ['shear_cl--bin'], 
                    num_bands=8, n_bin_pairs=15, l_min = 1.0, reorder = False, covariance = None,
    ...
    assert len(cl_vals) == len(to_write_headers), 'Mismatch between the number of cl_vals and file headers'
    assert len(cl_vals) == len(file_locs), 'Mismatch between the number of cl_vals and file locations to write'
    vals_to_read = list(cl_vals)
    vals_to_read.append(ell_val)
    # Read the input ell.
    mock_obj = km.read_kcap_values(mock_run = mock_run, mocks_dir = mocks_dir, mocks_name = mocks_name)
    ...
    if reorder is True:
        for i, bandpower_stacked in enumerate(bandpowers_stacked):
            bandpowers_stacked[i] = bandpower_stacked.reshape(n_bin_pairs,-1).T.flatten()
    ...
    if covariance is not None:
        ...
        for i, noisey_bandpower_stacked in enumerate(noisey_bandpowers_stacked):
            noisey_bandpowers_stacked[i] = np.random.multivariate_normal(noisey_bandpower_stacked, covariance)
        ...
        file_locs.extend([val.split('/')[0] + '/noisey_' + val.split('/')[1] for val in file_locs]) # adds the necessary file paths and headers for the files to write
    print("Writing bandpowers")
    mock_obj.write_to_tar(to_write = list(bandpowers_stacked), to_write_headers = to_write_headers, file_loc = file_locs)
